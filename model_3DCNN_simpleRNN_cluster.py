"""
Model 3DCNN + Simple RNN  cluster version
"""
...
class Complete_Model:
    ...
    def build_3DCNN(self):
        ...
        with tf.variable_scope(
            ...
            for i, num_filter in enumerate(self.config["num_filters"]):
                spatial_num_filter = num_filter
                temporal_num_filter = num_filter
            ...
            # print("CNN output raw shape as list", self.cnn_output_raw.shape.as_list())
            (
            ...
            # print(new_temporal_dim * cnn_height * cnn_width * num_filters)
            ...
            # print(self.cnn_output_flat.shape.as_list())
        ...
        """
        # THIS PART SHOULD BE PLACED IN PLACE OF THE CODE WITH VARIABLE SCOPE "2plus1Dconvolution" 
        # TO HAVE THE STANDARD 3D-CONVOLUTION OPERATION
        with tf.variable_scope("3Dconvolution", reuse=self.reuse, initializer=self.initializer,
                               regularizer=None):
            input_layer_ = self.input_layer
            for i, num_filter in enumerate(self.config['num_filters']):
                conv_layer = tf.layers.conv3d(inputs=input_layer_,
                                              filters=num_filter,
                ...
                pooling_layer = tf.layers.max_pooling3d(inputs=conv_layer, pool_size=[2, 2, 2],
    ...
    def build_RNN(self):
        ...
        with tf.variable_scope(
            ...
            self.rnn_output_raw, self.rnn_state = tf.nn.dynamic_rnn(
                cell=self.rnn_cell,
