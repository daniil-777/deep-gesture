"""
Training 3DCNN + Simple RNN
no 3DCNN pretraining
"""
...
from model_simple_transformer import Complete_Model
...
def main(config):
    ...
    training_file_pattern = os.path.join(config["data_directory"], "training-??-of-??")
    training_dataset = Dataset_3DCNN_plus_Simple_RNN(
        dataset_TFRecord=training_file_pattern,
    ...
    validation_file_pattern = os.path.join(
        config["data_directory"], "validation-??-of-??"
    )
    validation_dataset = Dataset_3DCNN_plus_Simple_RNN(
        dataset_TFRecord=validation_file_pattern,
        batch_size=config["batch_size"],
        window_size=config["window_size"],
        shuffle=False
        # normalize=config['normalize_data']
    ...
    with tf.name_scope("Training"):
        ...
        # tf.train.init_from_checkpoint("pretrained_models/3dcnn/1593162915_experiment-name/model-116349.data-00000-of-00001",{v.name.split(':')[0]: v for v in tf.contrib.framework.get_variables_to_restore()})
        # pretraining!!!
        # tf.train.init_from_checkpoint("pretrained_models/3dcnn/1593162915_experiment-name/",{v.name.split(':')[0]: v for v in tf.contrib.framework.get_variables_to_restore()})
